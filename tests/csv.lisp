...
(define-test read-into-buffer-until-test (:tags '(read-until))
  ...
  (with-input-from-string (in #?"test this thing")
    (let* ((s (make-string 4))
      ...
      (assert-eql 4 (cl-csv::read-into-buffer-until s in #\newline))
      (assert-eql 4 (cl-csv::read-into-buffer-until s in #\newline))
      (assert-eql 3 (cl-csv::read-into-buffer-until s in #\newline))
      (assert-error 'end-of-file (cl-csv::read-into-buffer-until s in #\newline))
      )))

(define-test fast-read-into-buffer-until-test (:tags '(read-until fast))
  ;; \r\l newline
  (with-input-from-string (in #?"test this\r\n thing")
    (let* ((s (make-string 80))
           (l (cl-csv::fast-read-into-buffer-until s in #?"\r\n")))
      (assert-eql 9 l)
      (assert-equal "test this" (displaced-sub-string s :end l))))
  ;; newline
  (with-input-from-string (in #?"t\nest this\n thing")
    (let* ((s (make-string 80))
      ...
      (assert-eql 1 l)
      (assert-equal "t" (displaced-sub-string s :end l))
      (let ((l (cl-csv::fast-read-into-buffer-until s in #\newline)))
        (assert-eql 8 l)
        (assert-equal "est this" (displaced-sub-string s :end l)))))
  ;; EOF
  (with-input-from-string (in #?"test this thing")
